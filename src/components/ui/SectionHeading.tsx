    ...
    <div
      className={
        centered
          ...
          : split
            ? "mb-12 grid gap-6 lg:grid-cols-[0.8fr_1fr]"
            : "mb-12 max-w-3xl"
      }
    >
      <div>
        <p className={`mb-4 text-sm font-medium ${labelColor}`}>{label}</p>
        <h2 className={`font-display text-4xl font-semibold leading-[0.98] md:text-5xl lg:text-6xl ${titleColor}`}>
          {title}
        </h2>
      </div>
      <p
        className={`text-base leading-7 md:text-lg ${introColor} ${
          split ? "lg:self-end lg:text-right" : ""
        }`}
      >
        {intro}
      </p>
